...
const SpecificFields = () => {
    ...
    const checkedFields = e => {
        if (e.currentTarget.checked) {
            ...
            const newArr = selectedFields.filter((item) => item !== e.target.value);
    ...
    useEffect(() => {
        if(selectedFields.length > 0) {
            specificFieldsFunction("&data_field_key=" + selectedFields.join(','))
        }
        else {
                ...
                {fields.map(field => {
                    return (
                        <div className="ml-2" key={field.key}>
                            <input id={`format-${field.key}`} type="checkbox" value={`${field.key}`} onChange={checkedFields}/>
                            <label className="ml-3 mb-0" htmlFor={`format-${field.key}`}>{field.description}</label>
                        </div>
                    )
                })}
            </div>
